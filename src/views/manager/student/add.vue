<template>
  <el-tabs class='container' id='add-student-page'>
    <el-tab-pane label='添加一群学生'>
      ...
      <el-table v-if='importStudentList' :data='pageStudentList'>
        <template slot='empty'>
          <el-alert
            id='table-emptyalert'
            title='没有数据'
        ...
        <el-table-column
          v-for='(item, idx) in Array(importStudentListMax)'
          :key='idx'
          align='center'
        ...
        >
          ...
          <template slot-scope='scope'>
            <div style='min-width: 100px'>{{scope.row[idx] || ''}}</div>
          </template>
        </el-table-column>
      </el-table>
      <div v-else>
        <el-alert title='请保证导入的数据包含学号列和姓名列' type='info' style='margin: 4px 12px 4px 12px;'></el-alert>
        <el-alert title='一次只能导入同一个教学班的学生' type='info' style='margin: 4px 12px 4px 12px;'></el-alert>
      </div>
      <el-pagination
        v-if='importStudentList'
...
export default {
  ...
  computed: {
    ...mapGetters(['user']),
    pageStudentList() {
      return this.importStudentList.slice(
  ...
  methods: {
    ...
    anotherSelected(type, idx) {
      ...
      console.log(
        '[AnotherSelected]',
        (type === 'name' && idx === sid_idx) ||
          (type === 'sid' && idx === name_idx)
      )
      return (
        (type === 'name' && idx === sid_idx) ||
        (type === 'sid' && idx === name_idx)
    ...
    onSubmitClicked() {
      ...
      if (sid_idx === -1 || name_idx === -1) {
        ...
        this.seletedSemester.year === undefined ||
    ...
    submitStudentList(list) {
      StudentViewModel.requestPostStudents(list)
        .then(res => {
          const unsucceed = list.length - res.succeed_ids.length
          const message =
            res.succeed_ids.length +
            '条学生数据成功导入，' +
            unsucceed +
            '条未导入'
          this.$message({
